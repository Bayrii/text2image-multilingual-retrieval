"""FastAPI application: serves text + voice retrieval to the React frontend.

Adds:
- CORS (open for dev; tighten in prod)
- Static image serving at /images/{filename}.jpg, mapped to data/images/
"""
...
from src.utils import get_logger, load_config, setup_caches
...
async def lifespan(app: FastAPI):
    ...
    voice = None
    if os.environ.get("VOICE_ENABLED", "1") not in ("0", "false", "False"):
        try:
            from src.voice import VoiceHandler
            ws = os.environ.get("WHISPER_SIZE", "small")
            wd = os.environ.get("WHISPER_DEVICE", "cpu")
...
def create_app() -> FastAPI:
    ...
    images_dir = Path(load_config()["data"]["images_dir"])
